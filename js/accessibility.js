class Accessibility {
  ...
  choiceElement() {
    if (
      document
        .querySelector(this.elementName)
        .classList.contains("font-increase") == false
    ) {
      document.querySelector(this.elementName).classList.toggle("active");
      document
        .querySelector(this.elementDoc)
        .classList.toggle(this.elementToogle);
    } else {
      this.countClick();
    }
    this.saveSelect();
  ...
  saveSelect() {
    ...
    if (document.querySelector(this.elementName).classList.contains("active")) {
      document
        ...
        .getAttribute(
          "aria-label",
          document.querySelector(this.elementName).querySelector("p")
            .textContent + " ׳׳•׳₪׳¢׳"
        );
...
let keyboardNav = new Accessibility(
  ".accessibility-menu .keyboard-activate",
  "keyboard-nav",
  "body"
);
let accessLink = new Accessibility(
  ".accessibility-menu .highlight-links",
  "access-link",
  "body"
);
let readable = new Accessibility(
  ".accessibility-menu .readable-font",
  "readable",
  "body"
);
let inverted = new Accessibility(
  ".accessibility-menu .invert",
  "inverted",
  "html"
);
let greyscale = new Accessibility(
  ".accessibility-menu .greyscale-mode",
  "greyscale",
  "html"
);
...
var fontSizeLevel = 0;
let allElementAccess = document.querySelectorAll(
  ".accessibility-menu li.access-element"
);

for (let i = 0; i < allElementAccess.length; i++) {
  let selectTab = allElementAccess[i];
  selectTab.addEventListener("click", () => {
    if (selectTab.classList.contains("keyboard-activate"))
      keyboardNav.choiceElement();
    if (selectTab.classList.contains("highlight-links"))
      accessLink.choiceElement();
    ...
    if (selectTab.classList.contains("greyscale-mode"))
      greyscale.choiceElement();
    if (selectTab.classList.contains("font-increase"))
      fontIncrease.choiceElement();
  });
}
...
document.addEventListener("keyup", function (e) {
  if (e.code == "Tab" || e.code == "Enter") {
    ...
    if (e.code == "Enter" && hassClassTab) {
      if (hassClassTab && selectTab.classList.contains("keyboard-activate"))
        keyboardNav.choiceElement();
      if (hassClassTab && selectTab.classList.contains("highlight-links"))
        accessLink.choiceElement();
      if (hassClassTab && selectTab.classList.contains("readable-font"))
        readable.choiceElement();
      if (hassClassTab && selectTab.classList.contains("invert"))
        inverted.choiceElement();
      if (hassClassTab && selectTab.classList.contains("greyscale-mode"))
        greyscale.choiceElement();
      if (hassClassTab && selectTab.classList.contains("font-increase"))
        fontIncrease.choiceElement();
    }
  }
});
...
(function ($) {
  ...
  $(document).on("click", ".accessibility-menu .font-increase", function () {
    $("h1,h2,h3,h4,h5,h6,p,div,a,span").each(function () {
      if (
        !(
          $(this).hasClass("access-element") ||
          $(this).parent().hasClass("access-element")
        )
      ) {
  ...
  $(document).on("keyup", function (e) {
    if (
      e.code == "Enter" &&
      document.activeElement.classList.contains("font-increase")
    ) {
      $("h1,h2,h3,h4,h5,h6,p,div,a,span").each(function () {
        if (
          !(
